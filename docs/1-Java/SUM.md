...
## Instant 时间戳
`Instant now = Instant.now();` - `now.getEpochSecond();` // 秒 - `now.toEpochMilli();` // 毫秒
...
- `ScheduledThreadPool`：可以定期调度多个任务（可定期反复执行） - FixedRate 是指任务总是以固定时间间隔触发，不管任务执行多长时间 - FixedDelay 是指上一次任务执行完毕后，等待固定的时间间隔，再执行下一次任务
...
数据库操作：增删改查
- **CRUD**：Create，Retrieve，Update 和 Delete
...
# JDBC Batch
批量操作
- 需要对同一个 `PreparedStatement` 反复设置参数并调用 `addBatch()`
- 调用的不是 `executeUpdate()`，而是 `executeBatch()`，返回类型是 `int[]`
...
# 分类
## 创建型模式（5 种）
> 关注于对象的创建机制，使系统独立于如何创建、组合和表示对象。
### 单例模式
确保一个类只有一个实例，并提供一个全局访问点。
### 工厂方法模式
定义一个创建对象的接口，让子类决定实例化哪一个类。
### 抽象工厂模式
提供一个接口，用于创建相关或依赖对象的族，而无需指定它们具体的类。
### 建造者模式
...
- **接口隔离原则**：使用多个专门的接口，而不是一个单一的总接口。
- **迪米特法则**：一个对象应当对其他对象有尽可能少的了解。
- **合成复用原则**：尽量使用对象组合，而不是类继承。
```
