...
- **_二进制_** (binary)：0，1 ，满 2 进 1 。以 `0b` 或 `0B` 开头。
...
数组是 **_引用类型_** ，在使用索引访问数组元素时，如果索引超出范围，运行时将报错：
...
执行 `ns = new int[] { 1, 2, 3 };` 时，它指向一个 _新的_ 3 个元素的数组：
...
**_异或运算_** 的规则是，如果两个数不同，结果为 `1`，否则为 `0`：
