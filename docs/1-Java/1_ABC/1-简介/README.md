...
> [!warning] > $JDK = JRE + 开发工具集（例如 Javac 编译工具等）$  
...
Java 规定，某个类定义的 `public static void main(String[] args)` 是 Java 程序的固定入口方法，因此，==**_Java 程序总是从 `main` 方法开始执行_**== 。
...
**_第一步_**，在保存 `Hello.java` 的目录下执行命令 `javac Hello.java`：
...
**_第二步_**，执行 `Hello.class`，使用命令 `java Hello`：
...
1. Java 程序-编译-运行的过程
   ...
   - 运行：使用 `java` 命令解释运行我们的字节码文件。
     - 格式： `java 类名`
2. 在一个 Java 源文件中可以声明多个 `class`，但是 ==**_最多只能有一个类声明为 `public`_**== 。
