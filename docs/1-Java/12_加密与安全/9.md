...
我们使用非对称加密算法的时候，对于一个 `公钥-私钥` 对，通常是用公钥加密，私钥解密。
...
因此，私钥加密得到的密文实际上就是**数字签名**，要验证这个签名是否正确，只能用私钥持有者的公钥进行解密验证。使用数字签名的**目的**是 _为了确认某个信息确实是由某个发送方发送的，任何人都不可能伪造消息，并且，发送方也不能抵赖_。
